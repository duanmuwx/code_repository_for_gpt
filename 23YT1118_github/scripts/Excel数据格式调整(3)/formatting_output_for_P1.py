...
class ExcelFormat:
    ...
    def main(self):
        ...
        for row_num, is_match in enumerate(mask, start=1):
            for col_num in range(1, df.shape[1]+1):
                if (col_num > self.start_col and col_num % 4 == self.start_col % 4) or col_num == self.start_col:
                    cell = ws.cell(row=row_num, column=col_num)
        ...
        for row_num, is_match in enumerate(mask, start=1):
            ...
            for col_num in range(1, df.shape[1]+1):
                ...
                color_time += 1
                if (col_num > self.start_col and color_time % 4 == self.start_col % 4 and color_time != 0):
                    select_color_num += 1
        ...
        for index, column in enumerate(ws.columns):
            ...
            if index == self.start_col or index == (self.start_col + 1):
                adjusted_width = pixel_width_to_character_width(61*0.60)
            else:
                adjusted_width = pixel_width_to_character_width(61*0.55)
            ws.column_dimensions[column_letter].width = adjusted_width
        # 为特定列执行列宽
        ws.column_dimensions["A"].width = pixel_width_to_character_width(70)
        ws.column_dimensions["B"].width = pixel_width_to_character_width(42*0.8)
        ws.column_dimensions["C"].width = pixel_width_to_character_width(200*0.2)
        ws.column_dimensions["D"].width = pixel_width_to_character_width(200*0.2)
        ws.column_dimensions["E"].width = pixel_width_to_character_width(90*0.6)
        # 设置行高
...
if __name__ == "__main__":
    ...
    # Excel格式调整
    # excelFormat = ExcelFormat(file_path="./data/2023-P1试验分析结果-1107-Sort_v2.xlsx", sheet_name="Sheet 1", start_col=5)
    # excelFormat.main()
    # Excel的sheet划分及格式调整（调用ExcelFormat对象）
    # excel = Excel(file_path='./data/2023年-P1试验-杂交种产量及病害分析-20231104.xlsx', result_folder='./result/', sheet_name="Sheet 1", start_col=5)
    # excel = Excel(file_path='./data/1104result_by_23P1-Sort_v2.xlsx', result_folder='./result/', sheet_name="Sheet 1", start_col=5)
    excel = Excel(file_path='./data/2023-P1试验分析结果-1107-Sort_v2.xlsx', result_folder='./result/', sheet_name="Sheet 1", start_col=5)
    excel.main()