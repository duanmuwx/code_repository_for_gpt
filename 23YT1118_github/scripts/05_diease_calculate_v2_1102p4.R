...
for (i in 1:length(all_trait_name)){
  ...
  for (aoa in AOA.vec){
    ...
    if (all(is.na(cau.data$trait) )) {
      ...
      result <- aggregate(trait ~ Name, cau.data, FUN = function(x) c(mean=mean(x), max(x), min(x),length(x)))
      result$AOA=aoa
      new_df <- cbind(result$Name, result$AOA,result$trait)
      
      colnames(new_df) <- c("Name","AOA",paste0("mean_",trait),paste0("max_",trait),
...
saveRDS(result.all.trait, "Output/1102P_diease_by_19_23.rds")
...
tmp=readRDS("Output/1102P_diease_by_19_23.rds")
tmp.1=rbind(result.all.trait,tmp)
saveRDS(tmp.1, "Output/1102P_diease_by_19_23_.tmp.rds")
