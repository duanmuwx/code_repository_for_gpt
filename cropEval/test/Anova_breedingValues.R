...
GCA_SCA_Estimation <- function(bluehb,ped){
    ...
    asr2 <- asreml(trait ~ LF,
                   random = ~ idv(SIRE) + idv(DAM) + idv(FamNum), 
                   residual = ~idv(units),
                   data = bluehb)
    
    summary(asr2)$varcomp
    vs <- summary(asr2)$varcomp$component[1]
    vd <- summary(asr2)$varcomp$component[2]
    vf <- summary(asr2)$varcomp$component[3]
    
    h2 <- vpredict(asr2,h2~(V1+V2)/(V1+V2+V3+V4))
    print(h2)
    
    d2 <- vpredict(asr2,h2~V2/(V1+V2+V3+V4))
    print(d2)
    loglik2 <- asr2$loglik
    asr3 <- asreml(trait ~ LF,
                   random = ~ idv(SIRE) + idv(DAM),# + idv(FamNum), 
                   residual = ~idv(units),
                   data = bluehb)
    loglik3 <- asr3$loglik
    ...
    # ebv <- extracting(asr2,var=vs,individual = ind,term='vm(Name)*')
    # head(ebv)
    ...
    sebv <- extracting(asr2,var=vs,individual = ind,term='SIRE*')
    ...
    sebv <- sebv[!substr(row.names(sebv),1,3)=='DAM',]
    debv <- debv[!substr(row.names(debv),1,4)=='SIRE',]
...
#setwd(choose.dir())
#ainv <- readRDS('pedigree_P1_inbredlines_ainverse.rds')
...
bluelist <- readRDS('datafiles/phenotypes_P_all.rds')
blue <- bluelist[["SW"]] #SU #SW #EM #SpML


#ML
#traits <- c('MST','TWT','YLD','PHT','EHT',"GLS")#?

#SU
#traits <- c('MST','TWT','YLD','PHT','EHT',"PMD.")
##'Phash','Ehash','PMDhash',,'LDGhash',
##''LDG','NCLB','GLS''Rust',"SCLB"

#SW
traits <- c('MST','YLD','PHT','EHT',"NCLB","Rust","SCLB","Rust","SCLB")#","

#EM
#traits <- c('MST','YLD','PHT','EHT',"LDG","NCLB")
...
for (i in 1:length(traits)){
    ...
    blue1 <- blue[,c("Location","Field","Range","Pass","Set","Name",traits[i])]
    
    names(blue1)[names(blue1)==traits[i]]<-"trait"
    
    asrout <- GCA_SCA_Estimation(bluehb=blue1,ped=ped)
...
writexl::write_xlsx(exout,path='Output/Anova_EBV_SW.xlsx')#?

save.image('Output/Anova_breedingValues_SW.rdata')#?
#region<-c("ML","E","SU")
#writexl::write_xlsx(exout,path=paste0('blup_EBV_',region[],'.xlsx')
