...
pheno <- readRDS("datafiles/23phenos_TC2_1118reallocated.rds")   ## raw_data (a data frame) for Hybrid_Observations and parent_Observations
...
pedigree_P_inbredlines <- readxl::read_excel('datafiles/23TC2ped_2023.10.30.xlsx',sheet=1)
...
#saveRDS(out, file = "Output/23TC2parent_hybrid_obs.rds")

#calculate population mean
pheno_datclean=readRDS("Output/23TC2_datalist_bluelist1118.rds")  #for calculate population mean
...
blupebv = readRDS("Output/1118GCA_23TC2_by_Station.rds") #MST,YLD,PHT,EHT,TWT
#diease <- readRDS("Output/1031diease_by_23TC2.rds")#diease traits
...
type <- readxl::read_excel('datafiles/23TC2ped_2023.10.30.xlsx',sheet=2)

for (i in 1:nrow(type)) {
  result$GCA$Type[result$GCA$GenoID==type$DAM[i]] = type$DH分群[i]
}
dim(result$GCA)
result$GCA=result$GCA[,c(1:2,76,3:75)]
...
file_path <- "Output/23TC2result_GCA_1118.xlsx"
