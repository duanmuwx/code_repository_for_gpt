...
#filename <- '22YTE-C.xlsx'
#sheets <- c('22YTE')
#outfile <- 'Output/blue_E.csv'
...
d<-read.csv("22YTE-C.csv",header=T,sep=",",check.names = F)
...
for (trait in traits){
    ...
    if (trait=='LDG')w$abnormal <- 1
    if (trait=='NCLB')w$abnormal <- 1
...
bluefile <- 'Output/blue_E.xlsx'
h2file <- 'Output/h2_E.xlsx'
rdsfile <- 'Output/blue_E.rds'
...
pedfile <- 'Output/blue_E_ped.rds'
saveRDS(ped,pedfile)

writexl::write_xlsx(ped,path='Output/pedigree_E.xlsx')
