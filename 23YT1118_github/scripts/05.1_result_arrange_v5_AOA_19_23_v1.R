...
traits <- c('YLD14', 'MST', 'PHT', 'EHT',"TWT","RTLPCT", 
            "TDPPCT", "GSPPCT","STKLPCT","ERTLPCT","LRTLPCT", "STKRPCT",
            "PMDPCT",'NCLB','SCLB','GLS',"RSTSOU",
            "SHBLSC","PMDSC","EARTSC")

pheno <- readRDS("datafiles/19_23_P_phenos_1118.rds")   ## raw_data (a data frame) for Hybrid_Observations and parent_Observations
...
#openxlsx::write.xlsx(out, file = "Output/parent_hybrid_obs.xlsx")
#saveRDS(out, file = "Output/parent_hybrid_obs.rds")

#calculate population mean
pheno_datclean=readRDS("Output/19-23P_datclean1118.rds") #for calculate population mean
...
blupebv = readRDS("Output/1118GCA_SCA_by_19_23.rds") #MST,YLD,PHT,EHT,TWT
diease <- readRDS("Output/1118diease_by_19_23.rds")#diease traits
...
for (region in region_all) {
  if(region=="EMSP"){
    ...
    ctrlmsample = 'WC009'  # male
    ctrlfsample = 'V76-1'  # female
...
file_path <- "Output/19_23Presult_GCA_SCA_1118.xlsx"
#modify(data=result,sheets=names(result))

#saveRDS(result,file ="Output/19_23P_GCA_SCA_1029.rds")
openxlsx::write.xlsx(result,file ="Output/19_23Presult_GCA_SCA_1118.xlsx") 
